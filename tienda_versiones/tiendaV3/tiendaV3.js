//se importan los datos del json 
    ...
    do {
      ...
      try {
        datosProductos.forEach((objeto) => {
          ...
          if ((objeto.getCod_prd === valor) && (objeto.getCantidad_prd !== 0)) {
      ...
      if (sw) {
        alert("Codigo no existe o ya no hay existencias del producto que intenta comprar");
      }
    } while (sw)
    return valor;
  }
...
// decalracion de las variables usadas en el metodo 
    ...
    do {
      //evita que el usuario digite como cantidad 0 
      do{
        valor = Number(this.capturaDato('Digite el numero de unidades=>', "numero"));
        if(valor <= 0)
           alert('cantidad digitada invalida');
      }while(valor <= 0)
      ...
      if (valor <= this.getCantidad_com) {
        ...
        alert("Cantidad digitada excede las existencias. ¡Verifique!");
      };
    } while (sw);
...
class Tienda {
  ...
  //metodo utilizado para cargar los archivos de json al inventario
  cargarProductosArchivo(){
    let i=0;
//se verifica si el arreglo importado esta vacio
    if (datosArchivo.length > 0){
    //se recorre el arreglo importado
         datosArchivo.forEach(objeto => {
          //por cada objeto que encuentra se crea un nuevo producto 
          //el iterador sirve para llevar la cuenta de cuantos productos se han cargado
              i++;
            //se crea un objeto de la clase Producto
              let producto = new Producto();
            //se actualizan los valores de las propiedades con los datos importados del json 
            //se actualiza el codigo 
              producto.setCod_prd = objeto.codigoProducto.toUpperCase();
            //se actualiza el nombre
              producto.setName_prd = objeto.nombreProducto.toUpperCase();
            //se actualiza la cantidad en inventario
              producto.setCantidad_prd = objeto.inventarioProducto; 
            //se actualiza el precio
              producto.setPrecio_prd = objeto.precioProducto; 
            // se usa el metodo push para poner el objeto en el inventario
              this.#productos_tind.push(producto);
...
//se imortan cargan los productos del json 
tienda.cargarProductosArchivo(); 
